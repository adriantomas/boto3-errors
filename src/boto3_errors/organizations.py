...
class ConstraintViolationException(OrganizationsError):
    ...
    - ACCOUNT_NOT_ACTIVE_FOR_TRANSFER_RESPONSIBILITY: Your account setup isn't complete
      or your account isn't fully active to invite or accept a Billing Transfer
      invitation.
    ...
    - UNMET_BILLING_PREREQUISITE: Your current billing configuration is unsupported.
    ...
    - UNSUPPORTED_PRICING: Ineligible for Billing Transfer. Your organization is subject
      to a pricing agreement with Amazon Web Services that Billing Transfer does not
      support.
...
class HandshakeConstraintViolationException(OrganizationsError):
    ...
    - ORGANIZATION_FROM_DIFFERENT_SELLER_OF_RECORD: You can only join an organization
      that operates in the same Amazon Web Services partition as your account.
    ...
    - TARGET_ACCOUNT_VALIDATION_FAILURE: Billing transfer is not available for your
...
class InvalidInputException(OrganizationsError):
    ...
    - INVALID_END_DATE: The selected withdrawal date doesn't meet the minimum notice
      period required by your partner agreement. Visit Amazon Web Services Partner
      Central or contact your Amazon Web Services Channel Partner for help.
    ...
    - INVALID_PATTERN: You provided a value that doesn't match the required pattern. The
      service also validates your free-text field values against common cross-site
      scripting (XSS) patterns and rejects requests that contain matching values.
