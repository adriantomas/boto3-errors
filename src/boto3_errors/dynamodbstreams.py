...
class DynamoDBStreamsError(Boto3Error):
    _SERVICE = "dynamodbstreams"


class ExpiredIteratorException(DynamoDBStreamsError):
...
class InternalServerError(DynamoDBStreamsError):
    """An error occurred on the server side."""
    _ERROR_CODE = "InternalServerError"


class LimitExceededException(DynamoDBStreamsError):
...
class ResourceNotFoundException(DynamoDBStreamsError):
...
class TrimmedDataAccessException(DynamoDBStreamsError):
    """The operation attempted to read past the oldest stream record in a shard.

    In DynamoDB Streams, there is a 24 hour limit on data retention. Stream records
    whose age exceeds this limit are subject to removal (trimming) from the stream. You
    might receive a TrimmedDataAccessException if:
    ...
    _ERROR_CODE = "TrimmedDataAccessException"


EXCEPTIONS: dict[str, type[DynamoDBStreamsError]] = {
